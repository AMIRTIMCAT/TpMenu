...
-- Кнопка "Показать меню" (по центру, draggable)
local showMenuBtn = Instance.new("TextButton")
showMenuBtn.Name = "ShowMenuBtn"
showMenuBtn.Text = "Меню"
showMenuBtn.Size = UDim2.new(0, 120, 0, 48)
showMenuBtn.Position = UDim2.new(0.5, 0, 0.5, 0)
showMenuBtn.AnchorPoint = Vector2.new(0.5, 0.5)
showMenuBtn.BackgroundColor3 = Color3.fromRGB(0, 170, 255)
showMenuBtn.BackgroundTransparency = 0.08
showMenuBtn.TextColor3 = Color3.fromRGB(255,255,255)
showMenuBtn.Font = Enum.Font.GothamBold
showMenuBtn.TextSize = 24
showMenuBtn.ZIndex = 100
showMenuBtn.Visible = false
showMenuBtn.Parent = screenGui
local showMenuCorner = Instance.new("UICorner")
showMenuCorner.CornerRadius = UDim.new(1,0)
showMenuCorner.Parent = showMenuBtn

showMenuBtn.MouseEnter:Connect(function()
    showMenuBtn.BackgroundColor3 = Color3.fromRGB(0, 200, 255)
end)
showMenuBtn.MouseLeave:Connect(function()
    showMenuBtn.BackgroundColor3 = Color3.fromRGB(0, 170, 255)
end)
...
-- Drag'n'drop для showMenuBtn (по центру, draggable)
local showDragging, showDragInput, showDragStart, showStartPos
showMenuBtn.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        showDragging = true
        showDragStart = input.Position
        showStartPos = showMenuBtn.Position
        input.Changed:Connect(function()
            if input.UserInputState == Enum.UserInputState.End then
                showDragging = false
            end
...
showMenuBtn.InputChanged:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
        showDragInput = input
    end
end)
UserInputService.InputChanged:Connect(function(input)
    if input == showDragInput and showDragging then
        ...
        showMenuBtn.Position = UDim2.new(showStartPos.X.Scale, showStartPos.X.Offset + delta.X, showStartPos.Y.Scale, showStartPos.Y.Offset + delta.Y)
    end
end)
...
local lastShowMenuBtnPos = UDim2.new(0.5, 0, 0.5, 0)
local function setMenuVisible(visible)
    -- Показываем/скрываем меню и тень
    frame.Visible = visible
    shadow.Visible = visible
    -- Кнопка "Показать меню" видна только когда меню скрыто
    showMenuBtn.Visible = not visible
    menuHidden = not visible
    if not visible then
        -- Сохраняем позицию, если пользователь уже таскал кнопку
        lastShowMenuBtnPos = showMenuBtn.Position
    else
        -- При открытии меню возвращаем кнопку в центр (или в последнее место, если таскали)
        showMenuBtn.Position = lastShowMenuBtnPos
    end
end
